...
export default function PendingLoanApplications({ onBack }) {
  ...
  try {
    ...
    payments = res.data.map((p) => parseFloat(p.amountPaid));
  } catch (err) {
  ...
  let remainingBalance = principal;
    ...
    let approvalDate = loan.createdAt;
  ...
  for (let i = 1; i <= months; i++) {
    ...
    // Adjust last month to ensure balance goes exactly to 0
    if (i === months) {
    ...
    scheduleData.push({
      ...
      remainingBalance: parseFloat(remainingBalance.toFixed(2)), // balance before deduction
      dueDate: new Date(approvalDate.getFullYear(), approvalDate.getMonth() + i, approvalDate.getDate()),
    });

    // Subtract principal for next month
  ...
  const handleAction = async (id, action) => {
    try {
      ...
      const endpoint =
        action === "approve"
          ? `http://localhost:8000/api/loans/loan/${id}/approve`
          : `http://localhost:8000/api/loans/loan/${id}/reject`;
    ...
    <div className="flex-1 bg-white rounded-lg shadow-lg p-3 relative">
      ...
      <div className="max-w-auto p-6">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-600 mt-6 border-t border-gray-300 pt-4">
      ...
      {selectedLoan && (
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
          <div className="bg-white rounded-2xl w-[700px] max-h-[85vh] overflow-y-auto shadow-2xl relative p-8">
            ...
            <h2 className="text-2xl font-bold text-center text-[#7e9e6c] mb-4">
            ...
            <div className="space-y-3 text-gray-700">
              ...
              <h3 className="text-xl font-bold text-[#56794a] mb-2">
  ...
  <p className="text-center text-red-600 font-semibold mt-4">
    Only SuperAdmins can approve or reject loans.
  </p>
)}
