    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-12 px-4">
      ...
      <div className="relative w-full max-w-3xl bg-white rounded-xl shadow-2xl overflow-auto p-6 z-60">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => onClose && onClose()} className="text-xl text-gray-600">&times;</button>
        </div>
        ...
        <div className="overflow-auto border rounded border-gray-200">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                sorted.map((r) => (
                  <tr key={r.id ?? r._id ?? `${r.memberId}-${r.date || r.createdAt}-${Math.random()}`} className="border-t">
